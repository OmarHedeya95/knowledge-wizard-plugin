...
var TextInputModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName("Input");
...
var VCWizardPlugin = class extends import_obsidian3.Plugin {
  async onload() {
  ...
  async add_hashtags(status) {
    ...
    for (let item of files) {
      ...
      if (file_ready_for_hashtags(file_content)) {
        new import_obsidian3.Notice(`Adding Hashtags to: ${item.basename}`);
  ...
  async find_similar_ideas(editor, view) {
    ...
    new import_obsidian3.Notice("Search in progress...");
  ...
  async register_file_change(file, type) {
    ...
    if (type == "modified" /* modified */) {
      ...
      new import_obsidian3.Notice(`${base_name} has been deleted`);
      ...
      if (file_path.contains("Readwise")) {
        new import_obsidian3.Notice(`${base_name} has been created`);
  ...
  async index_new_and_modified_files() {
    ...
    fs.readFile(storage_path, async (err, data) => {
      ...
      let files_to_modify;
      new import_obsidian3.Notice("Will read changed files now..");
      ...
      try {
        ...
        new import_obsidian3.Notice("No new notes to index");
        this.status.setText("\u{1F9D9}: Knowledge Wizard ready");
      ...
      let counter = 0;
      try {
        for (let [key, value] of Object.entries(files_to_modify)) {
          if (value.change_type == "new" /* new */ || value.change_type == "modified" /* modified */ || value.change_type == "deleted" /* deleted */) {
            new import_obsidian3.Notice(`Indexing file: ${key}`);
            counter = counter + 1;
        ...
        new import_obsidian3.Notice("There was an error while indexing!");
        this.status.setText("\u{1F9D9}: Knowledge Wizard ready");
        this.status.setAttr("title", "Knowledge Wizard is ready");
        return;
      }
      new import_obsidian3.Notice("Finished indexing!");
      if (Object.entries(files_to_modify).length > counter) {
        new import_obsidian3.Notice("Some files were NOT indexed! Run the command again");
  ...
  async index_vault() {
    ...
    let vault_path = this.settings.vaultPath;
    const plugin_path = scriptPath_AI;
    new import_obsidian3.Notice("Started indexing the full vault!");
    this.status.setText("\u{1F9D9}: Knowledge Wizard indexing...");
    this.status.setAttr("title", "Wizard is indexing your vault...");
  ...
  async market_research(industry, editor) {
    ...
    this.status.setText("\u{1F9D9} \u{1F50E}: Knowledge Wizard doing market research...");
    this.status.setAttr("title", "Wizard is researching the market");
    const research_result = await launch_python(pythonPath, scriptPath, scriptName, args);
    let dict = JSON.parse(research_result);
  ...
  async market_map(industry, editor) {
    ...
    console.log(plugin_path);
    var args = [industry, plugin_path];
    this.status.setText("\u{1F9D9} \u{1F50E}: Knowledge Wizard mapping the market...");
    this.status.setAttr("title", "Wizard is researching the market");
    const research_result = await launch_python(pythonPath, scriptPath, scriptName, args);
    let dict = JSON.parse(research_result);
  ...
  async url_research(url, editor) {
    ...
    this.status.setText(`\u{1F9D9} \u{1F50E}: Knowledge Wizard researching ${url}...`);
...
var VCWizardSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Python Virtual Environment Path").setDesc("The path to python virtual environment").addText((text) => text.setPlaceholder("Enter path").setValue(this.plugin.settings.pythonPath).onChange(async (value) => {
      console.log("PythonPath: " + value);
      this.plugin.settings.pythonPath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Pinecone API Key").setDesc("Your Pinecone API Key").addText((text) => text.setPlaceholder("Enter key").setValue(this.plugin.settings.pineconeKey).onChange(async (value) => {
      console.log("Pinecone key: " + value);
      this.plugin.settings.pineconeKey = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Pinecone Index Name").setDesc("Your Pinecone Index Name").addText((text) => text.setPlaceholder("Enter name").setValue(this.plugin.settings.pineconeIndexName).onChange(async (value) => {
      console.log("Pinecone Index Name: " + value);
      this.plugin.settings.pineconeIndexName = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Pinecone Envrionment Name").setDesc("e.g: us-east1-gcp").addText((text) => text.setPlaceholder("Enter environment").setValue(this.plugin.settings.pineconeEnv).onChange(async (value) => {
